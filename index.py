...
if __name__ == "__main__":
    if VALIDATION: input(RED + 'IMPORTANT!\nCréez une application sur Discord : https://discord.com/developers/applications\nDans "Rich presence" puis dans "Art assets", téléchargez 2 fichiers nommés "big" et "small" pour les images grande et petite\nDans "Informations générales", le "nom" est le nom de votre Rich Presence, et copiez l\'"ID DE L\'APPLICATION".\nAppuyez sur Entrée pour continuer.' + WHITE)
    ...
    class Server(BaseHTTPRequestHandler):
        def do_GET(self):
            ...
            self.wfile.write(bytes('</style></head><body><div class="testbox"><form onsubmit="event.preventDefault(); event.stopPropagation(); save()"><div class="banner"><h1>Configure ton activité</h1><h1 style="padding-top: 70px;">en direct !</h1></div>', "utf-8"))
            self.wfile.write(bytes('<div class="item"><p>Première ligne</p><input type="text" placeholder="Première ligne de votre activité" maxlength="128" value="%s" id="details" /></div>' % config['details'], "utf-8"))
            self.wfile.write(bytes('<div class="item"><p>Deuxième ligne</p><input type="text" placeholder="Deuxième ligne de votre acivité" maxlength="128" value="%s" id="state" /></div>' % config['state'], "utf-8"))
            self.wfile.write(bytes('<div class="item"><p>Texte de survol de la grande image</p><input type="text" maxlength="128" value="%s" id="large_text" /></div>' % config['large_text'], "utf-8"))
            self.wfile.write(bytes('<div class="item"><p>Texte de survol de la petite image</p><input type="text" maxlength="128" value="%s" id="small_text" /></div>' % config['small_text'], "utf-8"))
            self.wfile.write(bytes('<div class="question"><p>Afficher le temps passé</p><div class="question-answer"><div><input type="radio" value="none" id="start_yes" name="starttime" %s/><label for="start_yes" class="radio"><span>Yes</span></label></div><div><input type="radio" value="none" id="start_no" name="starttime" %s/><label for="start_no" class="radio"><span>No</span></label></div></div></div>' % (show1, show2), "utf-8"))
            self.wfile.write(bytes('<div class="item"><p>Bouton 1</p><div class="name-item"><input type="text" id="button1-label" placeholder="Button text" maxlength="32" value="%s" /><input type="text" id="button1-url" placeholder="Button URL" value="%s" /></div></div>' % (btn1_text, btn1_url), "utf-8"))
            self.wfile.write(bytes('<div class="item"><p>Bouton 2</p><div class="name-item"><input type="text" id="button2-label" placeholder="Button text" maxlength="32" value="%s" /><input type="text" id="button2-url" placeholder="Button URL" value="%s" /></div></div>' % (btn2_text, btn2_url), "utf-8"))
        ...
        def do_POST(self):
            ...
            print('Données sauvegardées, mise à jour du RPC dans quelques secondes...')
            save(json_data)
    ...
    print('Configuration fichiers, Vérification des champs...')
    AppIdToReset = False
    if VALIDATION: AppIdToReset = input("Appuyez sur « Entrée » pour lancer ou saisissez le nouvel ID pour changer l'ID de l'application : ")
    ...
    try:
        ...
        if VALIDATION:
            inputAppID = input("L'ID de l'application n'a pas été trouvé dans la configuration, veuillez le coller ici : ")
    ...
    def save(newconfig, save=True):
        ...
        print('RPC mit à jour avec succès !')
        
    save(config, False)


    webServer = HTTPServer((hostName, serverPort), Server)
    print("\nOuvrez la configuration Web ici :  http://%s:%s" % (hostName, serverPort))
    print('============= LOGS =============')
